...
Pinned **2.0.0** via [`build/MotusNetPackages.props`](build/MotusNetPackages.props) (NuGet publish pending). Until Motus.Core **2.0.0** is on nuget.org, build with `-p:UseMotusNetProjectReference=true` or `./build.ps1 -UseLocal` ([`build/MotusNetLocal.props`](build/MotusNetLocal.props)); CI checkouts `lasaths/Motus.NET` as a sibling and Prefer UseLocal. Motus.Core **1.8.0** is live — CI NuGet jobs override to that pin until **2.0.0** ships (**1.9.0** NuGet index may lag). Yak `motus` identity is **2.0.0** — pack with `-Yak` / `pack-yak.sh`; **do not** production `yak push` until Motus.NET **2.0.0** NuGet is live and human auth is present.
