...
Pinned **1.8.0** via [`build/MotusNetPackages.props`](build/MotusNetPackages.props) — default Release restores NuGet. For Motus.NET tip / close-open-dev, use sibling or in-repo `Motus.NET` via `-p:UseMotusNetProjectReference=true` or `./build.ps1 -UseLocal` ([`build/MotusNetLocal.props`](build/MotusNetLocal.props)); CI Prefer UseLocal jobs checkout `lasaths/Motus.NET` as a sibling. Yak `motus` is **not** on Package Manager yet — install from source / `-Yak` pack only (first public Yak target is **2.0.0**).
