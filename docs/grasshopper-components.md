...
| Motus Walking Hex | BodyR, Coxa/Femur/Tibia, stance angles, BodyZ; optional full `Q` (18) | Robot (Plan tip-path = one leg), full 18-DOF `Js`, preview Meshes, Support polygon — **not** Stewart |
...
**Motus Walking Hex** is a **separate** walking / mobile hexapod (6× coxa/femur/tibia), **not** `Family=stewart`. Motus.NET has no dedicated walking-hex planner; this component builds a branching kinematic tree for TreeFK + a mithi-style viewport preview (pink body, orange legs, COG, support polygon). **Motus Plan** / Joint State use the **tip path of one leg** only (same contract as Joint Table); the other five legs are preview-only. Example: `09_walking_hexapod.ghx`.
