...
Host-readiness checklist for Motus.Grasshopper against Motus.NET **1.8.0** on nuget.org.

**Default path:** NuGet pin (`build/MotusNetPackages.props`). **CI / tip path:** `-UseLocal` sibling Motus.NET (tracks Motus.NET master ahead of the pin). **Yak:** pack-only — `motus` is **not** on Rhino Package Manager (first public Yak target is **2.0.0**).
...
- [ ] `node scripts/verify-motus-net-pin.mjs` — props pin = **1.8.0**; Motus.Core **1.8.0** on nuget.org; docs must not claim publish-pending / UseLocal-only.
