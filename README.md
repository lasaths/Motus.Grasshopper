...
| Wires Rhino geometry ↔ Motus types | Host-agnostic .NET libraries ([NuGet](https://www.nuget.org/profiles/lasaths) pin **2.0.0**, Prefer UseLocal until published) |
...
- .NET 8 SDK
- Motus.NET **2.0.0** pin ([`build/MotusNetPackages.props`](build/MotusNetPackages.props)). Until that version is on nuget.org, Prefer `./build.ps1 -UseLocal` (sibling project refs; CI does the same). Motus.Core **1.8.0** remains the last confirmed nuget.org index while **1.9.0** may lag.
...
| `MotusNetVersion` | Override NuGet pin (default `2.0.0`; Prefer UseLocal until published) |
