...
- .NET 8 SDK
- Motus.NET **2.0.0** pin ([`build/MotusNetPackages.props`](build/MotusNetPackages.props)). Until that version is on nuget.org, build with `./build.ps1 -UseLocal` (sibling project refs; CI does the same). Motus.Core **1.8.0** is live on nuget.org.
